...
- [Egonex-AI/Understand-Anything](https://github.com/Egonex-AI/Understand-Anything) (MIT) - the ideas of a dependency-ordered guided tour, a persistent-graph-derived onboarding doc, and diff-impact analysis over a codebase graph (their `/understand-onboard`, `/understand-diff`, and guided-tour features) informed `skills/research/repo-tour/` in this repository. No text or code was copied; `repo-tour` reads the output of this repository's existing `graphify` skill rather than re-implementing Understand-Anything's own scanning/extraction pipeline, since that capability already exists here.
