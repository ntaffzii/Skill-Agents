...
**Suggested chain for a daily/weekly market check**: market-breadth-analyzer + uptrend-analyzer → exposure-coach → (if opening new risk) position-sizer, gated by drawdown-circuit-breaker → close the loop with trader-memory-core after the trade closes.
...
- This is a starter set (8 of the source repo's 71 skills), chosen for long-term/dividend investing, disciplined swing trading, and general market awareness. Screener skills that need live technical/fundamental data (breakout screeners, earnings calendars, backtesting) are out of scope for this starter set — the source repo's `docs/en/skills/` catalog is the reference if/when those get added.
